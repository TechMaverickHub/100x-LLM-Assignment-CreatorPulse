...
class DailyRegistrationsAPIView(GenericAPIView):
    ...
    def get(self, request):
        ...
        return_data = {
            ...
            "data": [item['count'] for item in user_queryset]
        }
...
class ActiveUsersAPIView(GenericAPIView):
    ...
    def get(self, request):
        ...
        user_queryset = get_user_model().objects.filter(role_id=GlobalValues.USER.value, is_active=True).order_by('-created')

        # group by days and limit to 7 days
        user_queryset = user_queryset.values('last_login__date').annotate(count=Count('id')).order_by('-last_login__date')[:7]
